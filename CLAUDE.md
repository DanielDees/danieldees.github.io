...
- **`library.js`** — THE END: `grid2` gen (one vast room; perimeter walls, shelf runs that converge/anchor into walls, table islands, the central 3-cell desk), connectivity repair, type-aware queries (`libCollide` — tables passable only while crouched; `losCells2` — shelves block sight, tables/desk don't; best-effort `bfsPath2` that routes to the nearest standable cell when the target is unreachable), `buildLibrary()` (geometry, hanging fixtures pushed into `lights` with `fixY`/`wakeAt`, furniture, posters/cracks/"THE END" wall texts, vintage PCs, 16–22 floppy disks, the terminal), and `updateLibrary()` (the 45s post-first-pickup light drop, temporary whole-floor blackouts, decor-PC boot-and-die animations).
- **`spider.js`** — THE END's librarian: horse-sized spider mesh with a procedural 8-leg gait, and the hearing-driven AI in `updateSpider()`. Disc pickups alert it after a distance-scaled reaction time (1s at ≤10m → 10s at 60% of room span); stacked pickups before it starts moving cut the countdown by 1s each and add +0.5× speed; pickups while it's en route retarget it and add +0.5× (base = player sprint 8 m/s, cap 3×). Uncrouched movement alerts it strongly <20m / mildly <30m; crouched movement is silent. It cannot enter table cells — a hidden player gets a circling `stalk` that times out.
...
- **`monster.js`** — level-0 entity mesh, wander/investigate/alert/chase/hunt state machine in `updateMonster()`, sight/hearing checks, wall-knocking, proximity audio.
- **`player.js`** — `updatePlayer()`: movement, sprint/stamina, jump physics, AABB collision (level-aware: `libCollide` in THE END; uncrouching is blocked under a tabletop), camera/head-bob, the post-drop micro screen shake (`STATE.shakeAmp`), footsteps.
